...
**フロー図への展開**:
```
[号1に該当？] ─はい→ [号1該当] → [用途上やむを得ない？] ─はい→ 適合（例外）
      │                                    │
      └─いいえ→ [号2に該当？]               └─いいえ→ 本則確認へ
                   │
                   └─はい→ [号2該当] → [用途上やむを得ない？]
...
**重要**: 付随条件（「用途上やむを得ない」等）は各号判定の**後**に配置する。

### 5.2 各号の個別展開（詳細版）

各号を個別のdecisionノードに展開する際のルール：

#### 5.2.1 各号への該当判定（decision）
...
```json
{
  ...
  "type": "decision",
  "title": "劇場・体育館等か？",
  "description": "【第1号】劇場、映画館、演芸場等の客席、体育館、工場その他これらに類する用途",
  "related_articles": ["令::A112:P1:I1"]
}
```

#### 5.2.2 該当状態の記録（process）
...
#### 5.2.3 付随条件の判定（decision）

```json
{
  "id": "dec-use-unavoidable",
  "type": "decision",
  "title": "用途上やむを得ない？",
  "description": "当該部分が用途上やむを得ないものか（機能上区画できない等）",
...
| 機序図の状態 | フロー図の展開 |
|-------------|---------------|
| `items` 配列に詳細あり | 各号を個別のdecisionに展開（**詳細版**） |
| `items` 配列なし | 「各号のいずれかに該当？」を1つのdecisionに（簡略版） |
| 号に `sub_conditions` あり | 号の判定後に副条件の判定を追加 |

**原則**: 機序図に詳細があれば、それをフロー図に反映する。
