...
- [ ] **都市計画情報（用途地域、防火地域、法22条区域等）を[情報]ノードにしていないか**
- [ ] **都市計画情報を参照する[処理]ノードを独立して作成していないか**
- [ ] **適用範囲条件（区域内か等）は `compliance_logic.scope_condition` にのみ記載しているか**

```json
// ❌ 誤り: 公共データを[情報]や[処理]ノードにする
{
  "kijo_diagram": {
    "nodes": [
      { "id": "info-zone", "title": "敷地の区域区分", "type": "information" },
      { "id": "proc-zone-check", "title": "区域判定", "type": "process" }
    ]
  }
}

// ✅ 正しい: 公共データは compliance_logic.scope_condition にのみ記載
{
  "compliance_logic": {
    "scope_condition": {
      "operator": "AND",
...
**理由**: 法適合判定には法令知識や都市計画情報も必要だが、これらは個別申請案件の処理メカニズム側に含まれるべきであり、[情報]としては扱わない。
...
- [ ] 別表参照を具体的に展開したか
- [ ] 孤立ノードがないか
- [ ] **入力元のない[情報]ノードがないか**（全[情報]は[処理]から出力されるか確認）
